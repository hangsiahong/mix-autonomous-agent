...
${system_prompt}"
    ...
    # Learned reasoning examples — captured from user reactions (👍/👎) on
    # the bot's own messages and distilled by tools/distill_examples.py.
    # File is regenerated by cron; injection is just a file read here.
    # Placed AFTER persistent memory so good/bad worked examples are the
    # most-recent stylistic reference the model sees before the system rules.
    if [[ -f "brain/learned_examples.md" && -s "brain/learned_examples.md" ]]; then
        local _learned_raw
        _learned_raw=$(cat "brain/learned_examples.md")
        if [[ -n "$_learned_raw" ]]; then
            system_prompt="${_learned_raw}\n\n---\n\n${system_prompt}"
        fi
    fi
  fi
